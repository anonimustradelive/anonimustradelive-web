...
### 2026-09-07 — Punto Zerø reabre: checkout de Whop embebido en la landing
- **Segunda convocatoria: del lunes 21 al sábado 26 de septiembre de 2026**, un solo horario de 2:00 a 4:00 p. m. hora del este (GMT-4, sigue siendo horario de verano en septiembre). Inscripciones abiertas desde el 7 de septiembre
- **El pago ya no sale de la página.** Se embebe el checkout de Whop (`plan_XfCA5U7ELvdHd`, US$ 299.99, pago único) en el mismo hueco del bloque `.close` donde estaba el botón. El loader `js.whop.com/static/checkout/loader.js` va **una sola vez** en el `<head>`; el propio loader inyecta después un segundo script (`index.js`), que no es un duplicado nuestro
- **Los cuatro CTAs restantes ya no salen a Whop**: nav, hero, regalo y barra móvil apuntan a `#reservar`, que es la sección del embed. Un solo embed en toda la página
- **`puntozero/gracias/index.html` (nuevo):** página de retorno del pago, `return-url` = `https://anonimustradelive.com/puntozero/gracias`. Se hizo como carpeta con `index.html` (no `gracias.html`) para que esa URL exacta funcione sin depender de MultiViews ni de un rewrite. Lee `?status`: sin parámetro o `success` muestra la confirmación y el botón a `whop.com/puntozero`; `status=error` avisa de que no se cobró nada y devuelve a la landing. El caso bueno está pintado en el HTML, así que sin JS igual se ve la confirmación
- **Red de seguridad:** si un bloqueador impide cargar `js.whop.com`, el div del embed queda vacío y no habría forma de pagar. Un script comprueba a los 5 s si se montó el iframe y, si no, enseña un enlace al checkout directo. Con el embed funcionando esa nota no se ve
- **Día 6.** El sábado se dedica a **Breakout**, que en la primera convocatoria solo se entregaba como material. Tarjeta nueva en `.days` y una línea de CSS: `.days .day:last-child` pasó a `:nth-child(odd):last-child`, para que la reja de seis cierre sola en vez de dejar hueco. El marketing sigue diciendo "programa intensivo de 5 días" por decisión del dueño; el sexto se presenta como día añadido
- **Se rehízo el copy del regalo,** que decía "solo en esta edición" y "a los grupos siguientes ya no se les regala". Repetir esa promesa incumplida ante un público desconfiado costaba más que el regalo. Ahora dice lo que de verdad pasó: funcionó, le dieron un día en vivo y lo dejaron dentro sin subir el precio
- **Desglose de valor actualizado:** de "Cinco clases (10 horas), US$ 1,000" a "Seis clases (12 horas), US$ 1,200". Total US$ 2,620 → **2,820**; ahorro 2,320 → **2,520**
- **FAQ:** se cayeron tres preguntas que quedaron falsas. "¿Puedo cambiarme de tanda?" (ya no hay tandas), "¿Por qué no hay grabaciones?" (ahora sí se graban, aunque no se anuncia como argumento de venta) y "¿Por qué no hay testimonios?" (ya hubo un grupo)
- **Sección de reseñas (mismo día, después):** las **tres reales** de Whop, con su @ visible. Van entre "La prueba" y "Honestidad". Sin estrellas doradas a propósito: la paleta ya tiene tres colores con significado asignado y un cuarto código no aportaría nada; se pintan en blanco. **Ojo:** `<footer>` dentro de `<blockquote>` heredaba la regla global `footer { background... }` y salía una caja negra dentro de la tarjeta; se cambió a `<p class="rev-who">`
- **La sección son solo el título y las tres tarjetas.** El primer intento llevaba un párrafo explicando que eran "tres, no treinta" y un enlace "Verlas en Whop". El dueño quitó las dos cosas: justificar el número cansa al lector y el enlace saca a la persona del embudo justo antes del precio
- **Se eliminó el párrafo de "Honestidad"** que decía "no vas a encontrar testimonios: no hay todavía, porque nadie ha tomado este bootcamp". Era falso, y reescribirlo habría sido otra justificación de más: las reseñas ya están arriba
- **Se retiró la lista de espera** (HTML, CSS y JS). `lead.php`, `leads.json`, el `.htaccess` y la pestaña Leads del panel **siguen en pie** para la próxima vez que haya que cerrar. La versión completa del bloque está en el commit `a8589bf`
